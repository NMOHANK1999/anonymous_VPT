...
def train(
    ...
    for epo in range(epochs):
        ...
        for image, mask in val_data_loader:
            ...
            mask = mask.to(device)
            enc_out = image
            dec_out = decoder(enc_out).unsqueeze(0)           
            tp, fp , fn , tn = smp.metrics.get_stats(output= dec_out.squeeze(0), target= mask.round().int(), mode = "binary", threshold = 0.5)
            ...
            if epo % show_mask_every == 0:
                ...
                if len(image_np.shape) == 3:
                    image_np = image_np[0, :, :]
                channel_1 = Image.fromarray(mask.cpu().numpy()*255)
